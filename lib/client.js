	...
	bender.assert.beautified = {
		...
		html: function( expected, actual, optionsOrMsg ) {
			...
			if ( !options.skipCompatHtml ) {
				...
				expected = compatHtml( expected, options.noInterWS, sortAttributes, fixZWS, options.fixStyles, fixNbsp, options.noTempElements );
				actual = compatHtml( actual, options.noInterWS, sortAttributes, fixZWS, options.fixStyles, fixNbsp, options.noTempElements );
			}
